...
class Prenet(torch.nn.Module):
    def __init__(self, hparams):
        ...
        self._hparams = hparams
        dims = [self._hparams.n_frames_per_step * self._hparams.n_mel_channels] + self._hparams.prenet_dim
        ...
        def forward(self, x):
            for l in self.fc_layers:
                x = F.dropout(F.relu(l(x)), p=self._hparams.prenet_dropout, training=True)
...
class Decoder(torch.nn.Module):
    ...
    def get_go_frame(self, memory):
        ...
        decoder_input = Variable(memory.data.new(
            B, self._hparams.n_mel_channels * self._hparams.n_frames_per_step).zero_())
    ...
    def parse_decoder_inputs(self, decoder_inputs):
        ...
        decoder_inputs = decoder_inputs.view(
            ...
            int(decoder_inputs.size(1)/self._hparams.n_frames_per_step), -1)
    ...
    def decode(self, decoder_input):
        ...
        self.attention_hidden = F.dropout(
            self.attention_hidden, self._haparms.attention_dropout, self.training)
        ...
        self.decoder_hidden = F.dropout(
            self.decoder_hidden, self._hparams.decoder_dropout, self.training)
